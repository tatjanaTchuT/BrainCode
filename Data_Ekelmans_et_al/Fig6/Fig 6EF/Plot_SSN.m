function Plot_SSN(a,b,n,J,r,axlim,N,col,thick)
if length(J(1,:))==4
...
f = @(x,y) Jee*max(0,y)-Jei*aI*max(0,max(0,y)*detJ/Jei+Jii/Jei*(bE-x*sqrt(N)+max(0,y/aE).^(1/nE))+x*sqrt(N)*r-bI).^(nI)+x*sqrt(N)-bE-max(0,y/aE).^(1/nE);
fimplicit(f,axlim,'Color',col,'LineWidth',thick)
hold on
