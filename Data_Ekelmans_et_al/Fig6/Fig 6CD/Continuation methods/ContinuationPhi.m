function NU = ContinuationPhi(J_,nu0,r_,mu0,tau_,delta,MaxIt,N)
    ...
    it=1;
    cos=1;
    P=10;%reduced step size if the V vector seems to be veering
    while it<MaxIt
        ...
        nu(1)=nu(1)+V(2)*delta*exp(P*(cos-1));%guesses based on V
        mu=mu+V(1)*delta*exp(P*(cos-1));
