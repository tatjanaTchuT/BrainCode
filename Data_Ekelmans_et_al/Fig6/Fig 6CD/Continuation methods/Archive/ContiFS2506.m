function NU = ContiFS2506(J_,nu0,r_,mu0,tau_,delta,MaxIt,N)
    ...
    a1=tauSyn*max(X(1),1e-3);%In case nu is very close to 0, we cap nu*tau to 1 mHz
    a2=tauSyn*max(X(2),1e-3);
    F(1) = X(1)-getnu(sqrt(J(1,1)^2*X(1)*C(1,1)*(1-a1*log(1+1/a1))+J(1,2)^2*X(2)*C(1,2)+sigmaX(1)^2),tau(1),J(1,1)*X(1)*C(1,1)+J(1,2)*X(2)*C(1,2)+X(3),ff);
    F(2) = X(2)-getnu(sqrt(J(2,1)^2*X(1)*C(2,1)+J(2,2)^2*X(2)*C(2,2)*(1-a2*log(1+1/a2))+sigmaX(2)^2),tau(2),J(2,1)*X(1)*C(2,1)+J(2,2)*X(2)*C(2,2)+r*X(3),ff);
